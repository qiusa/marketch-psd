    ...
    <div class="wrap" v-if="!noPage">
      ...
      <div class="wrap-layer" :style="{
        ...
        <div id="sketch-marker" class="sketch-marker" :style="{
          ...
          <b class="skm-w" :w="clickData.width + 'px'"></b>
          <b class="skm-h" :h="clickData.height + 'px'"></b>
          <span class="skm-tl"></span>
          <span class="skm-tr"></span>
          <span class="skm-bl"></span>
          <span class="skm-br"></span>
        </div>
        <!-- 创建虚线导航灯塔 -->
        ...
        <div id="skm-r-top" class="skm-rule skm-r-top" v-if="moveData.moveData" :style="{
          ...
          height: markData.top.height + 'px'}">
          <span>{{markData.top.text}}px</span>
        </div>
        <div id="skm-r-right" class="skm-rule skm-r-right" :style="{
          ...
          width: markData.right.width + 'px'}">
          <span>{{markData.right.text}}px</span>
        </div>
        <div id="skm-r-bottom" class="skm-rule skm-r-bottom" :style="{
          ...
          height: markData.bottom.height + 'px'}">
          <span>{{markData.bottom.text}}px</span>
        </div>
        <div id="skm-r-left" class="skm-rule skm-r-left" :style="{
          visibility: markShow && this.markData.top.visible ? 'visible' : 'hidden',
          top: markData.left.top + 'px',
          left: markData.left.left + 'px',
          width: markData.left.width + 'px'}">
          <span>{{markData.left.text}}px</span>
        </div>
        <!-- 图层 -->
    ...
    <transition name="custom-classes-transition" enter-active-class="animated bounceInRight" leave-active-class="animated bounceOutRight">
      <div class="panel panel-active" id="panel" v-if="markShow" @click.stop="panelClick">
        ...
        <dl class="pa-block">
          ...
          <dd>
            <ul>
              <li><input :value="clickData.left + 'px'" readonly="">
                <label>X</label>
              </li>
              <li><input :value="clickData.top + 'px'" readonly="">
                <label>Y</label>
              </li>
              <li><input :value="clickData.width + 'px'" readonly="">
                <label>Width</label>
              </li>
              <li><input :value="clickData.height + 'px'" readonly="">
                <label>Height</label>
              </li>
            </ul>
          </dd>
        ...
        <dl class="pa-block pa-export" v-if="!(clickData.text && clickData.text.font)">
          ...
          <dd>
            ...
            <div class="export-btn">
              <a id="export-btn" target="_blank" :href="layerDir + clickData.name + '.png'" :download="clickData.name + '.png'" data-base="982C7C37-8BDE-4866-8316-09B03A2CA13A/43232933-2667-4A6E-B336-BFB3CC9F3619" class="panel-export">Export Activity Layer</a>
            </div>
          </dd>
        </dl>
      </div>
...
export default {
  ...
  computed: {
    ...
    showCode() {
      ...
      if (this.clickData.text && this.clickData.text.font) {
        ...
        code +=
          'width: ' +
          this.clickData.width +
          'px\r\n' +
          'height: ' +
          this.clickData.height +
          'px\r\n'
      }
  ...
  methods: {
    ...
    showRuler() {
      ...
      if (
        (data.ty >= data.sy && data.ty <= data.sy + data.sh) ||
        (data.sy >= data.ty && data.sy <= data.ty + data.th)
      ) {
        if (data.sx - (data.tx + data.tw) >= buffer) {
